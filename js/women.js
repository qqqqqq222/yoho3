$(function(){
	...
	$(".pic ul li").hover(function(){
 		$(this).stop(true).animate({width:"650px"},500).siblings().stop(true).animate({width:"100px"},500);
	})
...
// 列表
	...
	$.get("../JSON/women2.json",function(data){
		$.each(data[0],function(index,value){
		...
		$.each(data[1],function(index,value){
			...
			if(j>0 && j<4){
				$("<li><a href='###'><img src='"+value.url+"' alt='' /></a></li>").appendTo($("#one2"));
			}else{
				$("<li><a href='###'><img src='"+value.url+"' alt='' /></a></li>").appendTo($("#three2"));
			}
		})

		$.each(data[2],function(index,value){
			var m=index.replace(/img/,"");
			if(m>0 && m<4){
				$("<li><a href='###'><img src='"+value.url+"' alt='' /></a></li>").appendTo($("#one3"));
			}else{
				$("<li><a href='###'><img src='"+value.url+"' alt='' /></a></li>").appendTo($("#three3"));
			}
		})
		...
		$.each(data[3],function(index,value){
			...
			if(n>0 && n<4){
				$("<li><a href='###'><img src='"+value.url+"' alt='' /></a></li>").appendTo($("#one4"));
			}else{
				$("<li><a href='###'><img src='"+value.url+"' alt='' /></a></li>").appendTo($("#three4"));
			}
		})

		// 人气单品TOP100
		$.each(data[4],function(index,value){
			$("<dl><dt><img src='"+value.url+"' alt=''></dt><dd><p>"+value.describe+"</p><span>"+value.price+"</span></dd></dl>").appendTo($("#hotter"));
			
		})
		
		$.each(data[5],function(index,value){
			$("<dl><dt><img src='"+value.url+"' alt=''></dt><dd><p>"+value.describe+"</p><span>"+value.price+"</span></dd></dl>").appendTo($("#newest"));
